...
{
    /**
     * @Route("/email-logs", name="pimcore_admin_email_emaillogs", methods={"POST"})
     *
     * @throws \Exception
     */
    public function emailLogsAction(Request $request): JsonResponse
    {
        ...
        if ($documentId = $request->request->getInt('documentId')) {
        ...
        if ($filterTerm = $request->request->get('filter')) {
            if ($filterTerm == '*') {
                $filterTerm = '';
            }
     ...
     * @throws \Exception
     */
    public function showEmailLogAction(Request $request, ?Profiler $profiler): Response
    {
        if ($profiler) {
            $profiler->disable();
        }
        ...
        $emailLog = Tool\Email\Log::getById($request->query->getInt('id'));
    ...
    public function deleteEmailLogAction(Request $request): JsonResponse
    {
        ...
        $success = false;
        $emailLog = Tool\Email\Log::getById($request->request->getInt('id'));
    ...
    public function sendTestEmailAction(Request $request): JsonResponse
    {
        ...
        $mail = new Mail();
        $mailType = $request->request->get('emailType');

        if ($mailType === 'text') {
            $mail->text($request->request->get('content'));
        } elseif ($mailType === 'html') {
            $mail->html($request->request->get('content'));
        } elseif ($mailType === 'document') {
            $doc = \Pimcore\Model\Document::getByPath($request->request->get('documentPath'));

            if ($doc instanceof \Pimcore\Model\Document\Email || $doc instanceof \Pimcore\Model\Document\Newsletter) {
                $mail->setDocument($doc);

                if ($request->request->get('mailParamaters')) {
                    if ($mailParamsArray = json_decode($request->request->get('mailParamaters'), true)) {
        ...
        $toAddresses = \Pimcore\Helper\Mail::parseEmailAddressField($request->request->get('to'));
